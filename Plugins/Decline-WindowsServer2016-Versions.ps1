...
$UnsupportedVersions = @("1709","1803")
Function Invoke-SelectUpdatesPlugin{

    $DeclineUpdates = @{}
    If (!$UnsupportedVersions){Return $DeclineUpdates}

    $Windows2016Updates = ($ActiveUpdates | Where {$_.ProductTitles.Contains('Windows Server 2016') })

    #Loop through the updates and decline any that match the version.
    ForEach ($Update in $Windows2016Updates){

        #If the title contains a version number.
        If ($Update.Title -match "\(\d\d\d\d\)" -and (! (Test-Exclusions $Update))){

            #Capture the version number.
            $Version = $matches[0].SubString(1,4)

            #If the version number is in the list then decline it.
            If ($UnsupportedVersions.Contains($Version)){
                $DeclineUpdates.Set_Item($Update.Id.UpdateId,"Windows Server 2016 Version: $($Version)")
            }
        }
    }
